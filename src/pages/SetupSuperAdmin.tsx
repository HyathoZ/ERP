  ...
  const handleSetup = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorMessage =
          result.error instanceof Error ? result.error.message : "Erro ao criar superadmin. Tente novamente.";
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm space-y-8">
        <div className="flex flex-col items-center">
          <h2 className="text-2xl font-semibold tracking-tight text-card-foreground">Configuração Inicial</h2>
          <p className="text-sm text-muted-foreground">Criar conta de Super Administrador</p>
        </div>
