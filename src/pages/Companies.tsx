...
export function Companies() {
  ...
  const updateCompanyStatus = async (companyId: string, active: boolean, plan: string) => {
    try {
      await api.put(`/api/companies/${companyId}`, { active, plan });
      setCompanies(companies.map((company) => (company.id === companyId ? { ...company, active, plan } : company)));
    } catch (error) {
        ...
        {filteredCompanies?.map((company) => (
          <div key={company.id} className="rounded-lg border p-4">
            ...
            <div className="mt-4">
              <select
                value={company.plan}
                onChange={(e) => updateCompanyStatus(company.id, company.active, e.target.value)}
              ...
              <button
                onClick={() => updateCompanyStatus(company.id, !company.active, company.plan)}
                className={`rounded px-2 py-1 ${company.active ? "bg-red-500 text-white" : "bg-green-500 text-white"}`}
              >
